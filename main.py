...
import dtw as detewu
import tslearn.metrics as detewu2
...
def read_wav_and_do_mfcc(filename):
    with wave.open(filename, 'r') as waveFile:
        ...
        return mfcc(np.array(result, dtype=np.float32), rate, winlen=0.0125, winstep=0.005, numcep=3, nfilt=3,
                    nfft=512 * 2)
...
def build_shortest_path(matrix_of_cumulated_distances):
    ...
    while (a != 0 or b != 0):
        if a == 0:
            ...
            if matrix_of_cumulated_distances[a - 1, b - 1] <= matrix_of_cumulated_distances[a - 1, b] and \
                    matrix_of_cumulated_distances[a - 1, b - 1] <= matrix_of_cumulated_distances[a, b - 1]:
                a -= 1
                b -= 1
            elif matrix_of_cumulated_distances[a - 1, b] <= matrix_of_cumulated_distances[a - 1, b - 1] and \
                    matrix_of_cumulated_distances[a - 1, b] <= matrix_of_cumulated_distances[a, b - 1]:
                a -= 1
            elif matrix_of_cumulated_distances[a, b - 1] <= matrix_of_cumulated_distances[a - 1, b - 1] and \
                    matrix_of_cumulated_distances[a, b - 1] <= matrix_of_cumulated_distances[a - 1, b]:
                b -= 1
        result += [[a, b]]
...
def better_dtw(A, B, verbose=False):
    rank = detewu2.dtw(A, B)#, dist=euclidean)
    return rank
...
for test_filename in filenames:
    ...
    # print(features)
    aggregators = [get_kNN_aggregator(
        1)]  # ,get_kNN_aggregator(3),get_kNN_aggregator(5)]#,get_aggregator(np.min),get_aggregator(np.mean)]
